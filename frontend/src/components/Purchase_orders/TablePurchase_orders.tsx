...
import { loadColumns } from './configurePurchase_ordersCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';
import { dataGridStyles } from '../../styles';

import ListPurchase_orders from './ListPurchase_orders';

const perPage = 10;

const TableSamplePurchase_orders = ({
  filterItems,
  setFilterItems,
  filters,
}) => {
  ...
  const {
    ...
    loading,
    count,
    notify: purchase_ordersNotify,
    refetch,
  } = useAppSelector((state) => state.purchase_orders);
  ...
  useEffect(() => {
    if (purchase_ordersNotify.showNotification) {
      notify(
        purchase_ordersNotify.typeNotification,
        purchase_ordersNotify.textNotification,
      );
    }
  }, [purchase_ordersNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/purchase_orders/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/purchase_orders/purchase_orders-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {purchase_orders && Array.isArray(purchase_orders) && (
        <ListPurchase_orders
...
export default TableSamplePurchase_orders;
